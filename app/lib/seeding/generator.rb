...
class Seeding
  ...
  class Generator
    ...
    REGION_NAME_PREFIXES = %w{
      shadow evil underground bizarro negative mirror universe Grimdark
    }.freeze
    ...
    def self.facility_name
      ...
      case rand(0..6)
      when 0
        words << ECCENTRIC_FACILITY_ADJECTIVES.sample
        words << Faker::Science.science
        words << ORGANIZATIONAL_UNITS.sample
      when 1
        words << Faker::Science.modifier
        words << ELEMENTS.sample
        words << FACTORY_SYNONYMS.sample
      when 2
        words << ECCENTRIC_FACILITY_ADJECTIVES.sample if roll_d100 < 20
        words << Faker::Science.tool
        words << coinflip(ORGANIZATIONAL_UNITS.sample, FACTORY_SYNONYMS.sample)
      when 3
        words << Faker::Job.field
        words << coinflip(Faker::University.suffix)
      when 4
        words << Faker::IndustrySegments.industry
        words << BORING_FACILITY_TYPES.sample
      when 5
        words << Faker::Science.scientist if roll_d100 < 20
        words << "#{ELEMENTS.sample}-#{ELEMENTS.sample}"
        words << FACTORY_SYNONYMS.sample
      when 6
        words << COMBINATION_WORDS.sample if roll_d100 < 30
        words << DODGY_PROFESSIONS.sample
        words << " & "
        words << ECCENTRIC_FACILITY_ADJECTIVES.sample if roll_d100 < 40
        words << Faker::Job.field
        words << ORGANIZATIONAL_UNITS.sample
      end
